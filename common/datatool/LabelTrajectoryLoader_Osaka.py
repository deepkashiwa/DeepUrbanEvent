...
STEP = 5
mesh = Mesh('osaka', '500m')
inputDimension_poi = 40

mesh100 = Mesh('osaka', '100m')
...
def getStepLabelTrajectoryGrid(filePath='./common.generator.instances/osaka_output.csv'):
    print('started', time.ctime())
    TDB = loadLabelTrajectoryGrid(filePath, mesh)
    print('ended', time.ctime())

    wf = open('./common.generator.instances/osaka_label_grid_' + str(STEP) + '.csv', 'w')
    for oid in TDB:
        sequence = TDB[oid]
...
def getStepLabelTrajectoryLonLat(filePath='./common.generator.instances/osaka_output.csv'):
    print('started', time.ctime())
    TDB = loadLabelTrajectoryLonLat(filePath, mesh)
    print('ended', time.ctime())

    wf = open('./common.generator.instances/osaka_label_lonlat_' + str(STEP) + '.csv', 'w')
    wf_grid = open('./common.generator.instances/osaka_label_grid_' + str(STEP) + '.csv', 'w')
    cnt = 0
    N_TDB = len(TDB)
    for oid in TDB:
        sequence = TDB[oid]
...
def shuffleGridTrainTest(rate=0.3):
    filePath = './common.generator.instances/osaka_label_grid_' + str(STEP) + '.csv'
    df = pd.read_csv(filePath, header=None, dtype=str)
    ...
    trainData.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_train.csv', header=0, index=0)
    testData.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_test.csv', header=0, index=0)
...
def shuffleGridTrainValidateTest(validateRate=0.2, testRate=0.2):
    ...
    testData = df.loc[test]
    trainData.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_train.csv', header=0, index=0)
    validateData.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_validate.csv', header=0,
                        index=0)
    testData.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_test.csv', header=0, index=0)
...
def shuffleBothTrainValidateTest(validateRate=0.2, testRate=0.2):
    print(validateRate, testRate)
    gridFilePath = './common.generator.instances/osaka_label_grid_' + str(STEP) + '.csv'
    df_grid = pd.read_csv(gridFilePath, header=None, dtype=str)

    lonlatFilePath = './common.generator.instances/osaka_label_lonlat_' + str(STEP) + '.csv'
    ...
    trainData_grid.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_train.csv', header=0,
                          index=0)
    validateData_grid.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_validate.csv', header=0,
                             index=0)
    testData_grid.to_csv('./common.generator.instances/osaka_label_grid_' + str(STEP) + '_test.csv', header=0, index=0)

    trainData_lonlat = df_lonlat.loc[train]
    validateData_lonlat = df_lonlat.loc[validate]
    testData_lonlat = df_lonlat.loc[test]
    trainData_lonlat.to_csv('./common.generator.instances/osaka_label_lonlat_' + str(STEP) + '_train.csv', header=0,
                            index=0)
    validateData_lonlat.to_csv('./common.generator.instances/osaka_label_lonlat_' + str(STEP) + '_validate.csv',
                               header=0, index=0)
    testData_lonlat.to_csv('./common.generator.instances/osaka_label_lonlat_' + str(STEP) + '_test.csv', header=0,
                           index=0)
...
def shuffleGridPart(Unit=100000):
    ...
    wfs = [open('./common.generator.instances/osakalabelgrid_step_' + str(STEP) + '_part_' + str(i) + '.csv', 'w') for i
           in range(N // Unit + 1)]
    for i in range(N):
        part = i // Unit
        wf = wfs[part]
...
def getStepXSYS(trainOrtest='train'):
    filePath = './common.generator.instances/osaka_label_grid_' + str(STEP) + '_' + trainOrtest + '.csv'
...
def getGridStepXSYSTVT(trainOrvalidateOrTest='train'):
    filePath = './common.generator.instances/osaka_label_grid_' + str(STEP) + '_' + trainOrvalidateOrTest + '.csv'
    df = pd.read_csv(filePath, header=None)
    XS = df.iloc[:, 1:-1]
    YS = df.iloc[:, -1:]
    XS = XS.values
...
def getLonLatStepXSYSTVT(trainOrvalidateOrTest='train'):
    filePath = './common.generator.instances/osaka_label_lonlat_' + str(STEP) + '_' + trainOrvalidateOrTest + '.csv'
    df = pd.read_csv(filePath, header=None)
    XS = df.iloc[:, 1:-2]
    YS = df.iloc[:, -2:]
...
def setLonLatDisDegXSYSTVT(trainOrvalidateOrTest='train'):
    lonlatfilePath = './common.generator.instances/osaka_label_lonlat_' + str(
        STEP) + '_' + trainOrvalidateOrTest + '.csv'
    df_lonlat = pd.read_csv(lonlatfilePath, header=None)

    # gridfilePath = './common.generator.instances/osaka_label_grid_' + str(STEP) + '_' + trainOrvalidateOrTest + '.csv'
...
def setGridDisDegXSYSTVT(city, trainOrvalidateOrTest):
    lonlatfilePath = './common.generator.instances/' + city + '_label_lonlat_' + str(
        STEP) + '_' + trainOrvalidateOrTest + '.csv'
    df_lonlat = pd.read_csv(lonlatfilePath, header=None)

    gridfilePath = './common.generator.instances/' + city + '_label_grid_' + str(
        STEP) + '_' + trainOrvalidateOrTest + '.csv'
    df_grid = pd.read_csv(gridfilePath, header=None)
...
def getStepXSYSPart(part):
    filePath = './common.generator.instances/osakalabelgrid_step_' + str(STEP) + '_part_' + str(part) + '.csv'
    df = pd.read_csv(filePath, header=None)
    XS = df.iloc[:, 1:-1]
    YS = df.iloc[:, -1:]
    XS = XS.values
...
def genScaledOutsidePOI(city, meshSize, window):
    ...
    wfFileName = './common.generator.instances/ScaledGridPoIOutside_' + city + '_' + meshSize + '_' + str(
        window) + '.csv'
    R = []
    for id in range(mesh.lonNum * mesh.latNum):
...
def gentreePOI():
    POI100 = getPOI('osaka', '100m')
    wfFileName = './common.generator.instances/GridPoIVideo_' + 'osaka' + '_' + '1000m' + '.csv'
    wf = open(wfFileName, 'w')
...
def gettreePOI():
    treefilePath = './common.generator.instances/GridPoIVideo_' + 'osaka' + '_' + '1000m' + '.csv'
...
def getOutsidePOI(window=3):
    wfFileName = './common.generator.instances/GridPoIOutside_' + 'osaka' + '_' + '1000m' + '_' + str(window) + '.csv'
...
def getDisDeg(XS, YS, mesh=Mesh('osaka', '500m')):
