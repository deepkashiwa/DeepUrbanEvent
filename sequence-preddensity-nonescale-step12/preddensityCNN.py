...
def getModel(name):
    if name == 'CNN':
        ...
        vision_model.add(Conv2D(32, (3, 3), padding='same', input_shape=(HEIGHT, WIDTH, TIMESTEP)))
        vision_model.add(BatchNormalization())
        vision_model.add(Conv2D(32, (3, 3), padding='same'))
        vision_model.add(BatchNormalization())
        vision_model.add(Conv2D(32, (3, 3), padding='same'))
        vision_model.add(BatchNormalization())
        vision_model.add(Conv2D(CHANNEL * TIMESTEP, (3, 3), activation='relu', padding='same'))
        return vision_model
    else:
        return None
...
def testModel(name, testData):
    ...
    inputXS = np.squeeze(XS.swapaxes(4, 1))
    YS_pred = train_model.predict(inputXS)
    YS_pred = np.expand_dims(YS_pred, axis=1)
    YS_pred = YS_pred.swapaxes(1,4)
...
MODELNAME = 'CNN'
### NEW3
KEYWORD = EVENT + 'tokyo' + meshTokyo.size + '5minpop-nonescale' + 'STEP' + str(TIMESTEP)
PATH = '../model' + KEYWORD
